	...
	if (!$conf[$cpart]["_nw_pcp"]) {
		...
		if (!$nodefaults) {
			...
			if ($cpart=="global") {
				...
				foreach ($GLOBALS["conf_vhosts_propagate"] as $key=>$pkey) if (!$conf[$cpart][$pkey]) $conf[$cpart][$pkey]=$conf["global"][$pkey];
...
function modules_init($method="init") {
	...
	foreach ($modules as $modclass)	for ($a=0;$a<count($modclass);$a++) {
...
function load_access_files($dir, &$access, $rec=0) {
	...
	if (is_array($z=$access_cache[$dir])) {
		...
		if (strpos($ndir.DIRECTORY_SEPARATOR, $GLOBALS["docroot"])===0) load_access_files($ndir, $access, $rec+1);
...
function access_query($key, $idx=false) {
	...
	switch ($ap) {
		...
		case "merge":
		$tmp=array_merge($conf["global"][$key], $conf[$GLOBALS["vhost"]][$key], $access["global"][$key]);
		break;
...
function nw_gethostbyaddr($ip) {
	...
	if ($ip!=$ns_lastreq[0]) $ns_lastreq=array($ip, @gethostbyaddr($ip));
...
function nw_apply_template($template, $args, $no_add=false) {
	...
	$thmid=access_query("servertheme", 0);
	$fname=$themes[$thmid]["_fname"];
	
	if ($thmid==$fname) $thmid=$themes[$thmid]["theme_id"];
	if ((!is_array($themes[$thmid])) && ($ltid=access_query("loadtheme", 0))) $fname=$ltid;
...
function nanoweb_reload($sig_no=SIGHUP) {
	...
	if (!$mypid) {
		...
		if (nanoweb_init($conffile)) {
			...
			techo("configuration file (".$conffile.") reloaded");

		} else {

			techo("configuration was not reloaded");
...
techo("aEGiS nanoweb/".VERSION." (C) 2002-2003 by sIX / aEGiS");
...
$os=(strpos(strtolower(PHP_OS), "win")===false)?"unix":"win32";
			...
			if ($sck_connected) {
				...
				if ($p1=$http_resource) {
					...
					foreach (access_query("_aliases") as $key=>$val) if (strpos(rtrim($http_uri, "/"), rtrim($key, "/"))===0) {
						...
						if ((is_dir($docroot.$http_uri)) && (substr($docroot.$http_uri, -1)!="/")) $pri_redir=$http_resource."/";
				...
				if ($rauths=access_query("authrequire")) {
				...
				if ($sck_connected) {
					...
					while (($buf=$lf->parser_get_output()) || $first_chunk) {
						...
						if ($lf->parser_eof() || !$sck_connected) break;
